...
function curve_fit(model::Function, xpts, ydata, p0, data; kwargs...)
	# construct the cost function
	f(p) = model(xpts, p, data) - ydata
	lmfit(f,p0; kwargs...)
end

function curve_fit(model::Function, xpts, ydata, wt::Vector, p0, data; kwargs...)
	# construct a weighted cost function, with a vector weight for each ydata
	# for example, this might be wt = 1/sigma where sigma is some error term
	f(p) = wt .* ( model(xpts, p, data) - ydata )
	lmfit(f,p0; kwargs...)
end

function curve_fit(model::Function, xpts, ydata, wt::Matrix, p0, data; kwargs...)
